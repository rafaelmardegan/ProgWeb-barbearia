...
function addTeste(){
	$.post("./../backend/controller/servicosController.php",{
		operacao: "listar"
	  ...
	  function(data,status){
		...
		resp.forEach(element => {
			...
			<img class="card-img-top" src="${element.diretorio}${element.nomeImg}" alt="Imagem de capa do card">
			<div class="card-body">
			  <h5 class="card-title text-center">${element.nomeImgHash}</h5>
			  <p class="card-text" >${element.descricao}</p>
			</div>
			<div class="card-footer">
			  <a class="btn btn-primary btn-block" onclick="showModalPopUp1()" role="button">Agendar</a>
			  <a class="btn btn-secondary btn-block" href="${element.link}" role="button">Tirar dúvidas</a>
			</div>
		  </div>`);
		});
	  });
